...
stickpack.litstick_huds = {}

stickpack.litstick_add_hitmarkers = function(player)
    local function add_hud_and_play_noise(player)
        local name = player:get_player_name()
        if not stickpack.litstick_huds[name] then
            stickpack.litstick_huds[name] = {}
        end
        minetest.sound_play("hitmarker_fx", {
            to_player = name,
            gain = 0.5,
        })
        table.insert(stickpack.litstick_huds[name], player:hud_add({
            hud_elem_type = "image",
            position = {x = math.random(450,550)/1000, y = math.random(450,550)/1000},
            text = "hitmarker.png",
            scale = {x=2, y=2},
        }))
    end
    for i=1, stickpack.litstick_hitmarker_count do
        minetest.after(i/10, add_hud_and_play_noise, player)
    end
end

stickpack.litstick_remove_hitmarkers = function(player)
    local name = player:get_player_name()
    if stickpack.litstick_huds[name] then
        for i in pairs(stickpack.litstick_huds[name]) do
            player:hud_remove(stickpack.litstick_huds[name][i])
        end
        stickpack.litstick_huds[name] = nil
    end
end
...
stickpack.register_stick("litstick", {
    ...
    on_use = function(itemstack, player, pointed_thing)
        ...
        if pointed_thing.type == "object" and pointed_thing.ref:is_player() and (not stickpack.limited[name]) then
            ...
            -- Add hitmarkers
            stickpack.litstick_add_hitmarkers(player)

            -- Hemp/sand spawner
            pos.y = pos.y + 1
            stickpack.create_following_particle_effect(hit_player_name, 10, {
                quantity_per_second = 10,
                texture = ((stickpack.family_friendly and "default_sand.png") or "farming_hemp_leaf.png"),
                min_size = 1,
                max_size = 5,
                gravity = 0.2,
            })

            -- Play smoke weed everyday/darude sandstorm
            minetest.sound_play(((stickpack.family_friendly and "darude") or "swed"), {
                pos = pos,
            ...
            -- Append glasses
            stickpack.apply_tempskin(11, hit_player, "mlg_glasses.png")

            -- Undo all the things and remove limiter
            minetest.after(11, function(player, name)
                stickpack.litstick_remove_hitmarkers(player)
